      ...
      <div className="h-32 flex items-center justify-between px-4 py-3">
        ...
        <div className="flex-1 flex justify-end">
          <div className="flex items-center space-x-6">
            <NavLink to="/flights" icon={<Plane className="h-5 w-5" />} label="Flights" />
            <NavLink to="/destinations" icon={<Map className="h-5 w-5" />} label="Popular Destinations" />
            <NavLink to="/schedule" icon={<Calendar className="h-5 w-5" />} label="Schedule" />
...
function NavLink({ to, icon, label }: { to: string; icon: ReactNode; label: string }) {
  return (
    <Link to={to} className="flex items-center space-x-1 px-3 py-2 rounded-md hover:bg-honeydew/10 transition-colors">
      {icon}
      <span>{label}</span>
    </Link>
  );
